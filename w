...
<audio id="insert_heat" src="btn heat click.mp3"></audio>
<audio id="insert_metal" src="btn metal click.mp3"></audio>
<audio id="insert_luna" src="btn luna click.mp3"></audio>
<audio id="insert_trigger" src="btn trigger click.mp3"></audio>
<audio id="insert_fang" src="btn fang click.mp3"></audio>
...
function playMemoryInsert(type){
  const a=document.getElementById("insert_"+type);
  if(a){a.currentTime=0;a.play();}
  else{btnSound.currentTime=0;btnSound.play();}
}
...
henshinBtn.onclick=function(){
  if(!leftMemory||!rightMemory){alert("请选择两个记忆");return;}
  henshinSound.play();
  henshinClicked=true;
  checkUltimate();
  checkXtreme();
  ...
  let audio=document.getElementById(key)||document.getElementById(rightMemory+"_"+leftMemory);
  if(audio) audio.play();
};
